...
class ShopProduct extends Model
{
    ...
    {
        $sku = $this->sku;
        $colorCode = substr($sku, 5, 3);
        $color = \Illuminate\Support\Facades\DB::table('colors')
                    ->where('colorID', $colorCode)
                    ->value('color');
    ...
    {
        $sku = $this->sku;
        $sizeCode = substr($sku, 8, 2);
        $size = \Illuminate\Support\Facades\DB::table('sizes')
                    ->where('sizeID', $sizeCode)
                    ->value('size');
    ...
    */
    ...
    Upate stock, sold
    */
    public static function updateStock($product_id, $qty_change, $cancel = false)
{
    ...
    if ($item) {
        ...
        if ($cancel) {
            ...
            $item->sold -= $qty_change;
            $item->reserved -= $qty_change;
            $item->reserved = max(0, $item->reserved);
        } else {
            $item->stock -= $qty_change;
            $item->sold += $qty_change;
            $reserved_qty = $item->stock - $item->sold;
            $item->reserved = max(0, $reserved_qty);
        }

        $item->save();

        // Process build
        if ($item->kind == SC_PRODUCT_BUILD) {
            foreach ($item->builds as $build) {
                $productBuild = $build->product;
                $build_qty_change = $qty_change * $build->quantity;

                if ($cancel) {
                    $productBuild->stock += $build_qty_change;
                    $productBuild->sold -= $build_qty_change;
                    $productBuild->reserved -= $build_qty_change;
                    $productBuild->reserved = max(0, $productBuild->reserved);
                } else {
                    $productBuild->stock -= $build_qty_change;
                    $productBuild->sold += $build_qty_change;
                    $build_reserved_qty = $productBuild->stock - $productBuild->sold;
                    $productBuild->reserved = max(0, $build_reserved_qty);
                }
