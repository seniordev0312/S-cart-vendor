...
class ShopCartController extends RootFrontController
{
        ...
        switch ($instance) {
            case 'default':
                ...
                if ($product->allowSale()) {
                    ...
                    // Increment the `reserved` column.
                        $productInstance = ScShopProduct::find($id);
                        $productInstance->increment('reserved', 1);
    ...
    private function _orderSuccess()
    {
        ...
        // Increment the `reserved` column for each product in the order.
        foreach($orderInfo['details'] as $detail) {
            $productInstance = ShopProduct::find($detail['product_id']);
            $productInstance->increment('reserved', $detail['qty']);
        }
